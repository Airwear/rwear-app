...
use App\DataTransformers\TrainingDataTransformer as Transformer;
use App\Http\Controllers\Api\BaseApiController as BaseController;
use App\Http\Requests\Api\Trainings\ReportRequest;
use App\Http\Requests\Api\Trainings\StoreRequest;
...
use App\Services\Trainings\CreateService;
use App\Services\Trainings\StatisticsService;
use App\Services\Trainings\UpdateService;
use App\Services\Trainings\DeleteService;
...
class ResourceController extends BaseController
{
    ...
    public function index(Request $request): array
    {
        try {
            ...
            return $this->toApi(
                $this->dataTransformer->transform(data: $this->repository->forApi($request))
            );
     ...
     * @return array
     */
    public function report(ReportRequest $request, StatisticsService $service, string $slug): array
    {
        try {
            ...
            $method = $request->isMethod('GET') ? 'handleView' : 'handle';

            return $this->toApi(
                $this->dataTransformer->transform(data: $service->{$method}($model, $request->validated()))
            );
